...
class SaveParamsIterCallback(tf.keras.callbacks.Callback):
    ...
    def on_train_end(selfself, epoch, logs=None):
        ...
        with open(os.path.join("logs", 'history', 'train_val_params_[all_subs].pkl'), "wb") as pickle_file:
...
if __name__ == '__main__':
    ...
    reps=2
    subjects = map(lambda x: 'e'+str(x), list(range(1,11)) )

    ds_train = None
    ds_test = None

    for subject in subjects:
        dataset_name = f'dtu_td_cmaa_50ov/5s_all_{subject}'
