...
All_val_acc = {}
Train_val_params={}
Current_dataset = 'dtu_td_cmaa_50ov_5s'
...
class SaveUMap(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        y_labels=data_sample['att_gender']

        # Define marker styles for two classes
        unique_labels = np.unique(y_labels)
        markers = ['o', 'x']  # Circle for one label, Cross for the other
        colors = ['red', 'blue']  # Define colors for distinction
        ...
        for i, label in enumerate(unique_labels):
            ...
            plt.scatter(latent_2d[mask, 0], latent_2d[mask, 1], 
                        marker=markers[i], label=f'Class {label}', color=colors[i], s=40, alpha=0.7)
...
if __name__ == '__main__':
    ...
    dataset_names = ['dtu_td_cmaa_50ov_5s']
    ...
    ds_train_subs = [[2], [2,3],[2,3,4],[2,3,4,5],list(range(2,11)), list(range(2,19))]
    ds_test_subs = [[1] for _ in range(len(ds_train_subs))]
    ds_train_subs.extend([[1], [1,3], [1,3,4], [1,3,4,5], list(range(3,11))+[1], list(range(3,19))+[1]])
    ds_test_subs.extend([[2] for _ in range(6)])

    ds_train_subs =[[2], [2,3],[2,3,4],[2,3,4,5],list(range(2,11)), list(range(2,19))]
    ds_test_subs = [[1] for _ in range(len(ds_train_subs))]

    ds_train_subs =[[1], [1,3], [1,3,4], [1,3,4,5],list(range(1,10)), list(range(14,19))+list(range(15,19))]
    ds_test_subs = [[14] for _ in range(len(ds_train_subs))]

    ds_train_subs =[list(range(0,8))+list(range(9,19))]
    ds_test_subs = [[8] for _ in range(len(ds_train_subs))]

    ds_train_subs =[list(range(2,19))]
    ds_test_subs = [[1] for _ in range(len(ds_train_subs))]
    ...
    def merge_datasets_by_sub(dataset_name, sub_inds):
        ...
        for sub_ind in sub_inds:
            dataset_name_full = f'{dataset_name}/sub{sub_ind}'
    ...
    for i, (ds_train_sub, ds_test_sub) in enumerate(zip(ds_train_subs, ds_test_subs)):
        ...
        dataset_name=f'dtu_td_cmaa_50ov'
        ...
        model = AttnCrossAudioEEGSimsiam(d_aecross=320)
