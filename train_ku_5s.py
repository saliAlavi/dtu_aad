...
All_val_acc = {}
Train_val_params={}
Current_dataset = 'kuleuven_50ov_5s'
...
class SaveParamsIterCallback(tf.keras.callbacks.Callback):
    ...
    def on_train_end(self, epoch, logs=None):
        ...
        with open(os.path.join("logs", 'history', f'train_val_params_{Current_dataset}.pkl'), "wb") as pickle_file:
...
if __name__ == '__main__':
    ...
    dataset_names = ['kuleuven_50ov_5s']
    dataset_sub_names = [f'sub{i}' for i in range(2,19)]
        ...
        for dataset_sub_name in dataset_sub_names:
            ...
            for Curr_fold,(train_split, test_split) in enumerate(zip(train_splits, test_splits)):
                    for rep in range(reps):
                        ...
                        x_audio_m_ds = ds_train.map(lambda x: x['audio_0_ds'])
                        x_audio_f_ds = ds_train.map(lambda x: x['audio_1_ds'])

                        model = AttnCrossAudioEEGSimsiamKU(d_aecross=320)
                        ...
                        history = model.fit(
                                    ...
                                    callbacks=[reduce_lr_callback, save_best_acc_callback, tensorboard_callback, save_params_callback])
