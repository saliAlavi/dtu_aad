...
class TFEncoder(layers.Layer):
    def __init__(self, d_model=50, **kwargs):
        ...
        self.f_encoder = CNNEncoder(d_model=d_model)
    ...
    def call(self, x, training=False):
        ...
        h_f = self.f_encoder(x, training=training)
        # tf.print(tf.shape(h_f))
        # tf.print(tf.shape(h_time))
        # h_time = tf.reshape(h_time, tf.shape(h_time))
        h_time = tf.expand_dims(h_time, -1)
        h_f = tf.expand_dims(h_f, -1)

        # h_time = self.time_embedding(h_time)
        # h_f = self.f_embedding(h_f)

        # x = tf.concat([h_time, h_f], axis=-2)
        # x = self.selfAttn(x)
        x = self.crossAttn(h_time, h_f)
        x = self.flatten(x)
        return x
...
class ProbeHead(layers.Layer):
    def __init__(self, d_model=16,**kwargs):
        ...
        self.cls = tf.keras.Sequential([
            ...
            layers.Dense(100, activation='relu'),
            layers.Dense(50, activation='relu'),
            layers.Dense(BATCH_SIZE, activation='sigmoid'),
        ], name='classifier_head')
...
class AttnTFContrastive(Model):
    def __init__(self, d_model=16):
        super(AttnTFContrastive, self).__init__()
