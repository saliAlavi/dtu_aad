...
class FullModel(layers.Layer):
    def __init__(self, **kwargs):
        ...
        self.cross_attention = AECrossAttention(d_model=128)
...
class ClassifierHead(layers.Layer):
    def __init__(self, dropout_rate=0.5, **kwargs):
        ...
        # self.classifier = self.cls = tf.keras.Sequential([
        #     layers.Flatten(),
        #     layers.Dense(100, activation='relu'),
...
class ProbeHead(layers.Layer):
    def __init__(self, dropout_rate=0.5, **kwargs):
        ...
        self.dense_1 = layers.Dense(50, activation='relu')
        self.do_1 = layers.Dropout(rate=dropout_rate)
        self.dense_2 = layers.Dense(50, activation='sigmoid', use_bias=False)
...
class SimCLRLoss(tf.keras.losses.Loss):
    def __init__(self, reduction=tf.keras.losses.Reduction.AUTO, name='SimCLRLoss'):
        super(SimCLRLoss, self).__init__(reduction=reduction, name=name)
...
class SimCLRLoss2(tf.keras.losses.Loss):
...
class AttnCrossAudioEEGContr(Model):
    def __init__(self, d_model=16):
        super(AttnCrossAudioEEGContr, self).__init__()
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            data_0['eeg_csp'] =data_0['eeg_csp'] + tf.random.normal(tf.shape(data_0['eeg_csp']), mean=0, stddev=1)
            data_0['audio_m_ds']=data_0['audio_m_ds'] + tf.random.normal(tf.shape(data_0['audio_m_ds']), mean=0, stddev=1)
            data_0['audio_f_ds']=data_0['audio_f_ds'] + tf.random.normal(tf.shape(data_0['audio_f_ds']), mean=0, stddev=1)

            data_1['eeg_csp'] =  data_1['eeg_csp'] + tf.random.normal(   tf.shape(data_1['eeg_csp']), mean=0, stddev=1)
            data_1['audio_m_ds']=data_1['audio_m_ds'] + tf.random.normal(tf.shape(data_1['audio_m_ds']), mean=0, stddev=1)
            data_1['audio_f_ds']=data_1['audio_f_ds'] + tf.random.normal(tf.shape(data_1['audio_f_ds']), mean=0, stddev=1)

            feat_0 = self(data_0, contrastive=True, training=True)
            feat_1 = self(data_1, contrastive=True, training=True)

            loss = self.SimCLRLoss(feat_0, feat_1, y_t)
        ...
        with tf.GradientTape() as tape:
            ...
            # Compute the loss value
            # (the loss function is configured in `compile()`
            # pred = self.encoder_base(data)
            # pred = self.classifier_head(pred, training=True)
    ...
    def call(self, data, training=False, contrastive=False):
        ...
        if contrastive:
            ...
            x = self.probe(x, training=training)
        else:
            x = self.encoder_base(data)
