...
class AudioEncoder(layers.Layer):
    def __init__(self, source='audio_l_ds', **kwargs):
        super().__init__(**kwargs)
        self.source = source
...
class EncoderBase(layers.Layer):
    def __init__(self, d_aecross=320,**kwargs):
        ...
        self.audio_encoder_m = AudioEncoder(source='audio_l_ds')
        self.audio_encoder_f = AudioEncoder(source='audio_r_ds')
...
class AttnCrossAudioEEGSimsiamNJU(Model):
    def __init__(self, d_aecross=320):
        super(AttnCrossAudioEEGSimsiamNJU, self).__init__()
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            data_0['audio_l_ds']=data_0['audio_l_ds'] + tf.random.normal(tf.shape(data_0['audio_l_ds']), mean=0, stddev=noise_std)
            data_0['audio_r_ds']=data_0['audio_r_ds'] + tf.random.normal(tf.shape(data_0['audio_r_ds']), mean=0, stddev=noise_std)

            data_1['eeg_csp'] =  data_1['eeg_csp'] + tf.random.normal(   tf.shape(data_1['eeg_csp']), mean=0,    stddev=noise_std)
            data_1['audio_l_ds']=data_1['audio_l_ds'] + tf.random.normal(tf.shape(data_1['audio_l_ds']), mean=0, stddev=noise_std)
            data_1['audio_r_ds']=data_1['audio_r_ds'] + tf.random.normal(tf.shape(data_1['audio_r_ds']), mean=0, stddev=noise_std)
    ...
    def latent(self,data_inp):
        ...
        data['audio_l_ds'] = self.normalizer_audio_m(data['audio_l_ds'])
        data['audio_r_ds'] = self.normalizer_audio_f(data['audio_r_ds'])
        ...
        if contrastive:
            if training:
                ...
                data['eeg_csp'] = data['eeg_csp'] + tf.random.normal(tf.shape(data['eeg_csp']), mean=0,
                                                                     stddev=noise_std)
                data['audio_l_ds'] = data['audio_l_ds'] + tf.random.normal(tf.shape(data['audio_l_ds']), mean=0,
                                                                           stddev=noise_std)
                data['audio_r_ds'] = data['audio_r_ds'] + tf.random.normal(tf.shape(data['audio_r_ds']), mean=0,
                                                                           stddev=noise_std)
            p = self.encoder_base(data)
