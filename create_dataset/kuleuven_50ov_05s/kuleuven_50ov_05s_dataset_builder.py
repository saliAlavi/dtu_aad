...
class Builder(tfds.core.GeneratorBasedBuilder):
    ...
    def _generate_examples(self,split):
        ...
        for subject in subjects:
            ...
            n_trials = len(f_eeg['preproc_trials'][0])
            ...
            for idx_trial in range(n_trials):
                # if idx_trial==5:
                #     break
                trial = f_eeg['preproc_trials'][idx_trial][0][0]
                lr = int(trial[3][0] == 'L')
                att_track = trial[8][0][0] - 1
                eeg = tr[0][0][0][1]

                sr_eeg = trial[1][0, 0][10][0, 0]
                condition = int(trial[5][0]=='hrtf')

                track_0 = trial[4][0, 0][0]
                track_1 = trial[4][1, 0][0]
                ...
                track_0_ds = trial[11][0,0][0][:,0,0]
                track_1_ds = trial[11][0,0][0][:,0,1]

                TOTAL_TIME =track_0_ds.shape[0]/sr_eeg

                n_sects = int((TOTAL_TIME - (1 - OV) * T_SPAN) // (OV * T_SPAN))
                n_channels = eeg.shape[1]

                eeg_all_curr = np.zeros((n_sects, n_channels, int(SR_EEG*T_SPAN)), dtype=np.float32)
                audio_0_all_curr = np.zeros((n_sects, int(SR_DS_AUDIO*T_SPAN)), dtype=np.float32)
                audio_1_all_curr = np.zeros((n_sects, int(SR_DS_AUDIO * T_SPAN)), dtype=np.float32)
                ...
                for sect_idx in range(n_sects):
                    ...
                    eeg_all_curr[sect_idx,:,:] = eeg[sect_idx*STEP_SIZE_EEG:sect_idx*STEP_SIZE_EEG+int(SR_EEG*T_SPAN),:].transpose(1,0) #sample, channel, time
                    # print(f'eeg shape:{eeg_all_curr.shape}')
                    audio_0_all_curr[sect_idx, :] = track_0_ds[sect_idx*STEP_SIZE_SDAUDIO:sect_idx*STEP_SIZE_SDAUDIO+int(SR_DS_AUDIO*T_SPAN)]
                    audio_1_all_curr[sect_idx, :] = track_1_ds[sect_idx*STEP_SIZE_SDAUDIO:sect_idx*STEP_SIZE_SDAUDIO+int(SR_DS_AUDIO*T_SPAN)]
