...
class Builder(tfds.core.GeneratorBasedBuilder):
    """DatasetBuilder for ku_luven_welch dataset."""
    ...
    def _info(self) -> tfds.core.DatasetInfo:
        ...
        return tfds.core.DatasetInfo(
            ...
            features=tfds.features.FeaturesDict({
                'eeg': tfds.features.Tensor(dtype=np.float32, shape=self._builder_config.eeg),
        ...
        if subject > 0:
            ...
            for idx_data in range(n_data):
                ...
                for i_sect in range(n_sects):
                    ...
                    eeg_welch = np.zeros(self._builder_config.eeg, dtype=np.float32)
                    for ch in range(66):
                        eeg_welch[:, ch] = np.asarray(signal.welch(eeg_curr[:, ch], 64)[1])
